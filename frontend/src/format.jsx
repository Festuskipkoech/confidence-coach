...
export const FormattedMessage = ({ content }) => {
  const formatMathNotation = (text) => {
    ...
    return text
      ...
      .replace(/\\?\(([a-z0-9])\\?\)/gi, '<span class="font-serif italic text-indigo-800">$1</span>')
      
      // Functions like \(f(x)\)
      .replace(/\\?\(([a-z0-9]+\([a-z0-9]+\))\\?\)/gi, '<span class="font-serif italic text-indigo-800">$1</span>')
      
      // Expressions with ^ for superscript like \(x^2\)
      .replace(/\\?\(([a-z0-9])\^([0-9]+)\\?\)/gi, 
        '<span class="font-serif italic text-indigo-800">$1<sup class="text-xs">$2</sup></span>')
      
      // More complex expressions like \(x^2 + y^2 = 25\)
      .replace(/\\?\(([^)]+)\\?\)/g, (match, p1) => {
        return '<span class="font-serif italic text-indigo-800 bg-indigo-50 px-1 rounded">' + 
  ...
  const formatInlineText = (text) => {
    ...
    formattedText = formattedText
      // Bold + Italic: ***text***
      .replace(/\*\*\*(.*?)\*\*\*/g, '<span class="font-bold italic text-indigo-800">$1</span>')
      // Bold: **text**
      .replace(/\*\*(.*?)\*\*/g, '<span class="font-bold text-indigo-700">$1</span>')
      // Italic: *text*
      .replace(/\*((?!\*)[^*]+)\*/g, '<span class="italic text-indigo-600">$1</span>')
      // Inline code: `text`
      .replace(/`(.*?)`/g, '<code class="bg-indigo-50 border border-indigo-200 px-1.5 py-0.5 rounded font-mono text-sm text-indigo-800">$1</code>')
      // Highlight key terms
      .replace(/(implicit differentiation|differentiate|variables|function|equation|relationships)/gi, 
               '<span class="bg-indigo-100 text-indigo-800 px-1 rounded-sm">$1</span>');
      ...
      // Close out code blocks
        ...
        formattedSections.push(
          <div key={`code-container-${index}`} className="relative">
            <div className="absolute top-0 right-0 bg-indigo-700 text-white text-xs px-2 py-1 rounded-bl font-mono">
              {codeLanguage || 'code'}
            </div>
            <pre
              key={`code-${index}`}
              className="bg-gray-50 p-4 pt-8 rounded-lg my-4 font-mono text-sm overflow-x-auto border-l-4 border-indigo-500 shadow-sm"
            >
    ...
    sections.forEach((line, index) => {
      ...
      if (trimmedLine.match(/^---+$/)) {
        ...
        formattedSections.push(
          <div 
            key={`divider-${index}`} 
            className="border-b-2 border-indigo-200 my-6 relative"
          >
            <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 bg-white px-4 text-indigo-600 text-sm font-semibold">
              Section Break
            </div>
          </div>
        );
      ...
      if (trimmedLine.match(/^#\s+[^#]/)) {
        ...
        formattedSections.push(
          <h1
            key={`h1-${index}`}
            className="text-2xl font-bold text-indigo-900 mt-6 mb-4 pb-2 border-b border-indigo-200 flex items-center"
          >
            <span className="bg-indigo-700 text-white w-7 h-7 flex items-center justify-center rounded-full mr-3 text-sm">
              {sectionCount}
            </span>
        ...
        formattedSections.push(
          <h2
            key={`h2-${index}`}
            className="text-xl font-bold text-indigo-800 mt-5 mb-3 flex items-center"
          >
        ...
        formattedSections.push(
          <h4
            key={`h4-${index}`}
            className="text-base font-semibold text-indigo-600 mt-3 mb-2"
            dangerouslySetInnerHTML={{
              __html: formatInlineText(trimmedLine.replace(/^####\s+/, ''))
            }}
          />
      ...
      // CORE IDEA HIGHLIGHTING
        ...
        formattedSections.push(
          <div
            key={`core-idea-${index}`}
            className="bg-indigo-100 border-l-4 border-indigo-500 p-4 rounded-r-lg my-4"
          >
            <div className="flex items-center">
              <div className="font-bold text-indigo-800 mr-2">Core Idea:</div>
              <div 
      ...
      // UNORDERED LISTS
        ...
        currentSection.items.push(
          ...
          >
            <span className="w-5 h-5 min-w-5 flex items-center justify-center rounded-full bg-indigo-100 text-indigo-700 mr-3 mt-0.5">•</span>
            <span
              dangerouslySetInnerHTML={{
      ...
      // ORDERED LISTS
        ...
        if (!isSectionHeader) {
          ...
          currentSection.items.push(
            ...
            >
              <span className="w-6 h-6 min-w-6 flex items-center justify-center rounded-full bg-indigo-200 text-indigo-800 mr-3 text-sm font-medium">
                {bullet}
              </span>
              <span
                dangerouslySetInnerHTML={{
    ...
    return formattedSections.map((section, index) => {
      ...
      if (section?.type === 'unordered-list') {
        return (
          <ul
            key={`ul-${index}`}
            className="my-4 pl-2 list-none space-y-2 bg-gray-50 py-4 px-6 rounded-md border-l-2 border-indigo-300"
          >
            {section.items}
          </ul>
        );
  ...
  return (
    <div className="formatted-message prose prose-indigo max-w-none">
